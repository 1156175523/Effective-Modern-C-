...
​	这种强转类型的语法不仅限于生成代理类类型的初始化程序。当创建与初始化表达式生成的变量类型不同的变量时也有用。例如，计算公差值：
